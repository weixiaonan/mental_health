...
function timeFromNow($dateline) {
    ...
    }elseif($dateline >= strtotime(date("Y-m-d",strtotime("-1 day")))){
...
// 成功后，输出 json
function show ($code=0, $data=null, $msg='ok')
{
	$error = array(
			'code'=>$code,
			'msg' => $msg,
...
function json_msg($suc = true, $msg = "ok" ,$val = ""){
    $data = array(
        'success' => $suc,
        'message' => $msg,
        'dt' => $val,
...
function role_check($n_id){
    ...
    if(!empty($n_id)){
        ...
        $m = trim($_REQUEST['m']);
        $str = $d.'&'.$c.'&'.$m;
        $public_role = $ci->config->item('public_role');
        if(!in_array($str,$public_role)){
            $r_id = $userInfo['act_type'];
            $tab_a = $ci->config->item('role_tab')['tab_a'];
            $tab_g = $ci->config->item('role_tab')['tab_g'];
            $tab_n = $ci->config->item('role_tab')['tab_n'];
            $tab_r = $ci->config->item('role_tab')['tab_r'];
            $zs = $ci->loop_model->row_query("select count(a.id) as zs from $tab_a a,$tab_n n,$tab_r r where a.r_id=r.id and r.status=1 and r.id='{$r_id}' and n.id=a.n_id and n.status=1 and n.id='{$n_id}' ")['zs'];
            if($zs != 1){
                return false;
            }
        }
